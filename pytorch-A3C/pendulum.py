...
class PendulumEnv(gym.Env):
    ...
    def __init__(self, g=10.0):
        ...
        high_o = np.ones((1, stt_sz, stt_sz, stt_sz)) * self.max_atoms_count
        high_a = np.ones((1, stt_sz, stt_sz, stt_sz))
        low = np.zeros((1, stt_sz, stt_sz, stt_sz))
        # self.action_space = spaces.Box(low=0,  high=1 , shape=(3,), dtype=np.float32)
        self.action_space = spaces.Box(low=low, high=high_a, dtype=np.float32)
        self.observation_space = spaces.Box(low=low, high=high_o, dtype=np.float32)
