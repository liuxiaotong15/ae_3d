...
class OUNoise(object):
    def __init__(self, action_space, mu=0.0, theta=0.15, max_sigma=0.3, min_sigma=0, decay_period=10000):
        self.mu           = mu
        self.theta        = theta
    ...
    def reset(self):
        ...
        self.state = np.zeros(self.action_dim) * self.mu # xiaotong
        
    def evolve_state(self):
        x  = self.state
        # dx = self.theta * (self.mu - x) + self.sigma * np.random.randn(self.action_dim)
        dx = self.sigma * np.random.standard_normal(self.action_dim) # xiaotong
        return x + dx # xiaotong
        # self.state = x + dx
        # return self.state
    
    def get_action(self, action, t=0):
        ou_state = self.evolve_state()
        self.sigma = 0.00001 + self.max_sigma - (self.max_sigma - self.min_sigma) * min(1.0, t / self.decay_period)
